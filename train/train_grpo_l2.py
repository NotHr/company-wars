...
# ///
"""
BOARDROOM — GRPO Training Script (L2 — 7 companies, best checkpoint saving)

HF Jobs A10G (recommended):
    hf jobs uv run --flavor a10g-small train/train_grpo_l2.py --model-id google/gemma-4-E2B-it

HF Jobs T4:
    hf jobs uv run --flavor t4-medium train/train_grpo_l2.py --model-id google/gemma-4-E2B-it --fp16

KEY CHANGES vs L1 script:
1. L2 environment — 7 companies, PROPOSE_MERGER action
2. Best checkpoint saving — model saved whenever reward improves
3. Full 12-turn episode reward (carried over from L1 fix)
"""
...
# ------------------------------------------------------------------ #
# Inlined server/companies.py  (L2 — 7 companies)                     #
# ------------------------------------------------------------------ #
@dataclass
class SectorTrait:
    name: str
...
SECTOR_TRAITS: Dict[str, SectorTrait] = {
    ...
    "Biotech":    SectorTrait("Biotech",    sabotage_damage_multiplier=0.70),
}
...
L2_COMPANIES: List[Dict] = [
    ...
    {"name": "Ashen Reach Energy",   "sector": "Energy"},
    {"name": "The Cobalt League",    "sector": "Telecom"},
    {"name": "Verdant Pact Bio",     "sector": "Biotech"},
]

L2_STARTING_STATS = {
    "cash":         50_000_000.0,
    "market_share": round(100.0 / len(L2_COMPANIES), 4),  # 14.2857
    "stock_price":  100.0,
    "reputation":   0.70,
}

MARKET_SHARE_BASELINE = L2_STARTING_STATS["market_share"]


# ------------------------------------------------------------------ #
# Inlined server/game_logic.py  (L2 with all bug fixes applied)        #
# ------------------------------------------------------------------ #
@dataclass
class CompanyState:
    name: str
...
def resolve_turn(
    ...
    for name, action in actions.items():
        ...
        if action.action_type == "EARNINGS_CALL":
            ...
            merger_proposals[name] = action.action_target
    ...
    for proposer, target_name in merger_proposals.items():
        ...
        proposer_co = companies.get(proposer)
        if not target or not target.alive or not proposer_co or not proposer_co.alive:
            continue
        if target_name == proposer:
            continue
        mutual = merger_proposals.get(target_name) == proposer
        if mutual:
            # Mutual merger — absorb target
            ...
            # One-sided — penalise proposer slightly, give target small boost
            proposer_co.reputation = max(0.0, proposer_co.reputation - 0.25)
            rewards[proposer] -= 0.5
            target.market_share += 1.0
            target.reputation = min(1.0, target.reputation + 0.05)
            rewards[target_name] += 0.15
            # FIX: deduct from proposer to conserve total market share
            proposer_co.market_share = max(0.0, proposer_co.market_share - 1.0)
            processed_mergers.add(target_name)  # FIX: prevent multiple bonuses
    ...
    # Active partnership bonus — only alive partners
...
# ------------------------------------------------------------------ #
# Inlined server/boardroom_environment.py  (L2)                        #
# ------------------------------------------------------------------ #
...
def _build_prompt(primary, all_companies, leaderboard, active_partnerships,
                  emails_received, press_wire, turn, max_turns) -> str:
    if not primary:
        return "Game over."
    lines = [
...
# ------------------------------------------------------------------ #
# Companies set for reward validation                                  #
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
# Reward function — full 12-turn episode                               #
# ------------------------------------------------------------------ #
def reward_fn(prompts, completions, **kwargs) -> List[float]:
    ...
    for prompt, completion in zip(prompts, completions):
        ...
        # Environment reward — full 12-turn episode
        env_reward = 0.0
        try:
            action_pool = [
                ...
                BoardroomAction(action_type=atype, action_target=r)
                for atype in ("SABOTAGE", "PARTNERSHIP", "PROPOSE_MERGER")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir",          default="checkpoints/boardroom-grpo-l2")
    p.add_argument("--run-name",            default="boardroom-grpo-l2-v1")
...
class BestCheckpointTracker:
    """
    Saves the model whenever reward improves.
    This means the final saved model is the BEST one, not the last one.
    """
    def __init__(self, model, tokenizer, out: Path):
        self.model = model
    ...
    def check_and_save(self, step: int, reward: float) -> bool:
        if reward > self.best_reward:
            self.best_reward = reward
            self.best_step = step
            save_path = self.out / "best_checkpoint"
            save_path.mkdir(parents=True, exist_ok=True)
            self.model.save_pretrained(save_path)
            self.tokenizer.save_pretrained(save_path)
            print(f"\n[best] ✓ New best reward {reward:.4f} at step {step} → saved to {save_path}")
            # Save metadata
            (save_path / "best_info.json").write_text(json.dumps({
                "best_reward": reward,
                "best_step": step,
            }, indent=2))
            return True
...
def _wandb_finish(out: Path, summary: dict) -> None:
    try:
        ...
        best = out / "best_checkpoint"
        if best.exists():
            m = wandb.Artifact("best-lora-adapter", type="model")
            m.add_dir(str(best))
            wandb.log_artifact(m)
            print(f"[wandb] best checkpoint uploaded")
...
def _save_plots(history: list, out: Path) -> None:
    try:
        ...
        fig.suptitle("BOARDROOM GRPO — L2", fontsize=13)
        if losses:
...
def main():
    ...
    print(f"[config] model={args.model_id}  steps={args.max_steps}  "
          ...
          f"fp16={args.fp16}  env=L2 (7 companies)")
    ...
    # Best checkpoint tracker
    best_tracker = BestCheckpointTracker(model, tokenizer, out)

    print("[dataset] collecting L2 rollout prompts ...")
    ...
    def patched_step(*a, **kw):
        ...
        # Check for best checkpoint every step
        history = trainer.state.log_history
        last_reward = next(
            (h["reward"] for h in reversed(history) if "reward" in h), None
        )
        if last_reward is not None:
            best_tracker.check_and_save(step_counter[0], last_reward)

        # Refresh dataset every N steps
        if step_counter[0] % (args.refresh_every * 2) == 0:
            print(f"\n[refresh] step {step_counter[0]} — re-rolling L2 dataset ...")
    ...
    print(f"[train] starting {args.max_steps} steps on L2 environment ...")
    trainer.train()

    # Save final adapter
    model.save_pretrained(out / "lora_adapter_final")
    tokenizer.save_pretrained(out / "lora_adapter_final")
    print(f"[save] final adapter → {out / 'lora_adapter_final'}")
    print(f"[save] best adapter  → {out / 'best_checkpoint'} (reward={best_tracker.best_reward:.4f} at step {best_tracker.best_step})")
    ...
    summary = {
        ...
        "best_reward":        best_tracker.best_reward,
